...
# add logCPM count to the table of assays
logCPM_count <- edgeR::cpm(edgerres$data, log = TRUE, prior.count = 2)
aData <- assays(se$sg)
aData$logCPM <- logCPM_count[rownames(se$sg), colnames(se$sg)]

# row data
rD <- cbind(rowData(sg1), rowData(sg2))
rD <- rD[, !duplicated(colnames(rD))]
rowData(sg) <- rD
...
reducedData <- mds %>%
    ...
    dplyr::select(-one_of(colnames(colData(sg1))))
reducedData <- as.matrix(reducedData)

sce_gene <- SingleCellExperiment(assays = aData, 
                            rowData = rowData(sg),
                            colData = colData(sg),
