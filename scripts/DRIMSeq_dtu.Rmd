...
Here, we store the results on the gene level with the original data. The result table `DRIMSeq_res` is appended on the `rowData` of the original  `SummarizedExperiment` object `sg`. The updated `sg` could be feed to the R package `iSEE` to perform more exploratory and visual analysis.
...
The updated `st` could be feed to the R package `iSEE` to perform more exploratory and visual analysis.
```{r DRIMSeq-se-tx}
## add rows (NA) for genes that are filtered out (if any)
