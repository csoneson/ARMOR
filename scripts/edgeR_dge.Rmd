...
edgeR_resA <- lapply(seq_along(edgeR_res), FUN = function(x) {
  ...
  resX <- edgeR_res[[x]]
  resX <- resX %>% 
    dplyr::select(c("gene_id", "symbol", "logFC", "logCPM", 
                    "F", "FDR", "PValue")) 
  ...
  if (length(geneO) > 0) {
    ...
    resO$gene_id <- geneO
    resO$symbol <- rowData(sg)$symbol[match(geneO, rowData(sg)$gene_id)]
  ...
  rownames(resA) <- paste(resA$gene_id, resA$symbol, sep = "__")
...
```{r camera_text1, echo = FALSE, results = 'asis', eval = !is.null(genesets)}
cat("# Load gene sets 
...
```{r camera_text2, echo= FALSE, results = 'asis', eval = !is.null(genesets)}
cat("# Perform tests
...
```{r camera_text3, echo= FALSE, results = 'asis', eval = !is.null(genesets)}
...
```{r camera_text4, echo = FALSE, results = 'asis', eval = !is.null(genesets)}
